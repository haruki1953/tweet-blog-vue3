...
import type { BackendProfileStore, ProfileAddIconData } from '@/types'
import { useProfileStore, useStatesStore } from '@/stores'
import { useIconAddService } from '@/services'
import { profileIconUrl } from '@/utils'
import type IconSettingDialog from './IconSettingDialog.vue'
import type IconEditDialog from './IconEditDialog.vue'
...
const props = withDefaults(
  defineProps<{
    max?: number
    couldCancel?: boolean
    onUploaded?: (resData: ProfileAddIconData) => void
    initUuid?: string
  }>(),
  {
    max: 1,
...
const imageArray = computed(() => {
  return [...profileStore.externalIcons].sort(
...
onMounted(() => {
  if (props.initUuid !== undefined) {
    selectImgById(props.initUuid)
  }
})

const { isUploading, uploadImage } = useIconAddService({
  propsonUploaded
})
...
const refIconSettingDialog = ref<InstanceType<typeof IconSettingDialog> | null>(
  null
)
const refIconEditDialog = ref<InstanceType<typeof IconEditDialog> | null>(null)

// 限制图片显示个数
const imageShowLimitNum = ref(12)
const imageShowLimitAdd = () => {
  imageShowLimitNum.value += 12
}
...
<template>
  <div class="avatar-selector" ref="boxRef">
    <IconEditDialog ref="refIconEditDialog" v-model="model"></IconEditDialog>
    <IconSettingDialog ref="refIconSettingDialog"></IconSettingDialog>
    ...
    <div class="selector-botton-row">
      <div class="upload-button-group">
        ...
        <el-button
          ...
          @click="refIconSettingDialog?.open"
        ></el-button>
      </div>
      <div>
        <el-button
