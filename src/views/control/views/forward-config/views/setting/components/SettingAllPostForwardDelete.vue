...
import { postControlDeleteForwardAllPostApi } from '@/api'
...
const submit = async () => {
  ...
  try {
    const res = await postControlDeleteForwardAllPostApi()
    ...
    if (postForwardNum > 0) {
      ...
      sakiNotification({
        type: 'success',
        title: '删除成功',
        message: `已删除 ${postForwardNum} 条推文转发记录`
      })
    } else {
      sakiMessage({
        type: 'info',
...
<template>
  <div class="setting-excess-import-delete">
    <ConfirmContainer
      ...
      title="确认要删除全部推文转发记录吗？"
      confirmText="删除"
      confirmType="danger"
      size="small"
    >
      <div class="control-lable">删除全部推文转发记录</div>
