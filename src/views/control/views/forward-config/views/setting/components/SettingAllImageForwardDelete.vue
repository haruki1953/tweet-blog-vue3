...
import { postControlDeleteForwardAllImageApi } from '@/api'
...
const submit = async () => {
  ...
  try {
    const res = await postControlDeleteForwardAllImageApi()
    // const postForwardNum = res.data.data.postForward.count
    const imageForwardNum = res.data.data.imageForward.count
    if (imageForwardNum > 0) {
      // 删除转发记录后，帖子需重新加载
      postStroe.resetPostRequested()
      sakiNotification({
        type: 'success',
        title: '删除成功',
        message: `已删除 ${imageForwardNum} 条图片转发记录`
      })
    } else {
      sakiMessage({
        type: 'info',
...
<template>
  <div class="setting-excess-import-delete">
    <ConfirmContainer
      ...
      title="确认要删除全部图片转发记录吗？"
      confirmText="删除"
      confirmType="danger"
      size="small"
    >
      <div class="control-lable">删除全部图片转发记录</div>
